...
public class BankIdHandler extends AbstractHandler {
    public BankIdHandler(Printer printer) {
        super(printer);
    }
    ...
    public CommandInfo handle(Vector<String> tokens) {
        ...
        return new CommandInfo(tmp.id(), tmp.name(), tmp.amount(), number, tmp.catId(), tmp.type());
    }
}
